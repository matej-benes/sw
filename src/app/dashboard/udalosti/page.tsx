...
import type { Trida, User, Udalost } from '@/lib/types';
...
export default function ObecnaUdalostPage() {
  ...
  const udalostiCollection = useMemoFirebase(() => firestore ? collection(firestore, 'udalosti') : null, [firestore]);
  const { data: events, isLoading: eventsLoading } = useCollection<Udalost>(udalostiCollection);
  ...
  const handleDeleteEvent = (eventId: string) => {
      if (!firestore) return;
      deleteDocumentNonBlocking(doc(firestore, 'udalosti', eventId));
      toast({
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(handleSaveEvent)} className="space-y-6">
       ...
       <Card>
        <CardHeader>
            <CardTitle>Seznam událostí</CardTitle>
            <CardDescription>Přehled všech naplánovaných událostí.</CardDescription>
        </CardHeader>
        <CardContent>
            <Table>
                <TableHeader>
                ...
                <TableBody>
                    {eventsLoading && (
                        <TableRow><TableCell colSpan={4} className="text-center h-24">Načítání...</TableCell></TableRow>
                    )}
                    {!eventsLoading && events?.length === 0 && (
                        <TableRow><TableCell colSpan={4} className="text-center h-24">Žádné události k zobrazení.</TableCell></TableRow>
                    )}
                    {events?.map(event => (
                        <TableRow key={event.id}>
                            <TableCell className="font-medium">{event.nazev}</TableCell>
                            <TableCell>{format(new Date(event.datum), 'd.M.yyyy')} v {event.cas}</TableCell>
                            <TableCell>{event.typ}</TableCell>
                                 ...
                                 <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        ...
                                        <AlertDialogFooter>
                                            ...
                                            <AlertDialogAction onClick={() => handleDeleteEvent(event.id)}>Smazat</AlertDialogAction>
                                        </AlertDialogFooter>
                                    </AlertDialogContent>
                                </AlertDialog>
                            </TableCell>
